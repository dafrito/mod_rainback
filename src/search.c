...
{
    rainback_SearchResponse* resp = malloc(sizeof(*resp));
...
void rainback_generateSearchPage(rainback_Page* page, mod_rainback* rb, const char* uri)
{
    ...
    char* ampsep = index(uri, '&');
    char searchterm[MAX_URI_LENGTH + 1];
    switch(apr_unescape_url(searchterm, uri, ampsep ? ampsep - uri : strlen(uri), 0, 0, 1, 0)) {
    case APR_SUCCESS:
    case APR_NOTFOUND:
        break;
    default:
        marla_die(rb->session->server, "Failed to unescape URI to generate search page.");
    }

    char* term = 0;
    char* sep = strstr(searchterm, "?q=");
    if(sep) {
        term = sep + 4;
    }
    else {
    ...
    char esc[MAX_URI_LENGTH + 1];
    switch(apr_escape_entity(esc, term, strlen(term), 0, 0)) {
    case APR_SUCCESS:
    case APR_NOTFOUND:
        break;
    default:
        marla_die(rb->session->server, "Failed to encode URI to generate search page.");
    }
...
static marla_WriteResult readSearchForm(mod_rainback* rb, marla_Request* req, char* buf, size_t buflen, parsegraph_user_login* login)
{
    if(!login) {
    ...
    rainback_SearchResponse* resp = req->handlerData;
    rainback_SearchResponse_destroy(resp);
...
static marla_WriteResult readRequestBody(marla_Request* req, marla_WriteEvent* we)
{
    ...
    if(we->length == 0) {
        if(!strcmp(req->method, "POST")) {
            ...
            buf[len] = 0;
            marla_WriteResult wr = readSearchForm(resp->rb, req, buf, len, &resp->login);
...
void rainback_searchHandler(struct marla_Request* req, enum marla_ClientEvent ev, void* data, int dataLen)
{
    ...
    case marla_EVENT_MUST_WRITE:
        marla_killRequest(req, 500, "SearchHandler must not process write events.");
        break;
